...
# Funções e variáveis
plist_path = "/var/mobile/Library/Application Support/Flex3/patches.plist"
txt_path = "/var/mobile/Library/Application Support/Flex3/patches.txt"
...
if sys.argv[1] == "-i":
   ...
   print(f"[ {time.strftime('%Y-%m-%d %H:%M:%S')} ]: script_version: dopamine ")
   sys.exit()
...
if sys.argv[1] == "-s":
   print(f"\n\n\n[ {time.strftime('%Y-%m-%d %H:%M:%S')} ]: {plist_path} - OpenFile: success")
   ...
   while True:
        ...
        with open(info_file, 'a') as file:
            file.write(f"[ {time.strftime('%Y-%m-%d %H:%M:%S')} ]: {plist_path} :200: open")
